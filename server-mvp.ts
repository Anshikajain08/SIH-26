  ...
  if (cachedRun && cachedRun.events_json && cachedRun.matches_json) {
    ...
    try {
      for (let i = 0; i < rEvents.length; i++) {
        ...
        insEv.run(ev.id, documentId, ev.rawTextExcerpt, ev.normalizedActivityText, ev.discipline, ev.action, ev.objectOrTag, ev.eventDate, ev.status, ev.quantity ?? null, ev.unit ?? null, ev.extractionConfidence, 'GEMINI');
        const m = rMatches[i] || {};
