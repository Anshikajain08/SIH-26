...
async function matchEventEmbedded(event: ProgressEvent, actList: Activity[]): Promise<ReturnType<typeof matchEventToActivities>> {
  ...
  let eventVec: number[] | undefined;
  try {
    const res = await embedTexts([`${event.normalizedActivityText} ${event.rawTextExcerpt}`]);
    eventVec = res[0];
